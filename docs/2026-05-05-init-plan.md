...
Double-down icon helper removed; actions now use the context menu.
...
Notebook actions use the context menu.
...
export function renderProjectedNodes(args: {
  ...
  merged.each(function (node) {
    ...
    const iconGroup = group
      context menu handles notebook actions
...
notebook expand button removed
  color: var(--text-muted);
  cursor: pointer;
}

notebook expand hover state removed
