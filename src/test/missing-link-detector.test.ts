...
describe("missing notebook detection", () => {
  it("marks heading links as missing when the heading does not exist", () => {
    ...
    const missing = findMissingNotebookLinks({ app, doc, sourcePath: "maps/source.naotu" });
  ...
  it("treats existing block links as valid", () => {
    ...
    expect(isNotebookLinkMissing({ app, node, sourcePath: "maps/source.naotu" })).toBe(false);
  });
});
